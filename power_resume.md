# Devin Atkin, EIT

**Electrical Engineer in Training** | Calgary, AB

📧 devin@atkin.engineer | 📱 (403)-830-1177 | 🔗 [LinkedIn](https://linkedin.com/in/devin-atkin)

## Professional Summary

Electrical Engineer in Training with experience in power distribution design, cost estimation, and utility standards compliance, complemented by a strong analytical and automation background. Former Distribution Powerline Designer at ATCO Electric, with hands-on experience supporting real-world utility projects across northern Alberta. Currently completing a PhD in Electrical Engineering with strong emphasis on modelling, validation, and systems reliability.

## Core Competencies

### Power Systems & Distribution
- Distribution system design and service layouts
- Utility cost estimation and work package preparation
- Overhead and underground distribution concepts
- Electrical drawings and standards interpretation

### Engineering Tools
- AutoCAD (utility drawings and layouts)
- Microsoft Excel (estimates, analysis, reporting)
- Technical documentation and design packages

### Engineering Practices
- Design verification and review
- Standards-driven workflows
- Cross-disciplinary coordination
- Safety-aware design mindset

## Professional Experience

### Distribution Powerline Designer
**ATCO Electric** | January 2017 – December 2017

- Designed distribution service deployments across northern Alberta
- Produced cost estimates for projects ranging from $20k to $1M
- Prepared and reviewed design documentation for live utility projects
- Automated estimating workflows, improving consistency and turnaround time
- Operated within utility standards and approval processes

### Technical Operations & Researcher
**University of Calgary – I2Sense Laboratory** | May 2019 – February 2026

- Conducted applied engineering research on complex electrical systems
- Maintained shared laboratory equipment with uptime and safety requirements
- Developed internal tools for engineering workflows and data validation

### Programming Intern
**Eideticom** | 2024 (500 hours)

- Supported hardware system testing and validation
- Developed automated test infrastructure and documentation

## Education

**PhD in Electrical Engineering** (In Progress)
University of Calgary | Expected defense: Early 2026
Focus: System modelling, validation, and measurement

**BSc in Electrical Engineering** (Honours)
University of Calgary | Internship program participant

## Professional Memberships

- Engineer in Training, APEGA
- Member, IEEE
